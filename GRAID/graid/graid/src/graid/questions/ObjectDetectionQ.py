...
def sort_indices_left_to_right_with_vertical_tiebreak_using_width(
    centroids: List[Tuple[float, float]],
    widths: List[float],
    x_eps_factor: float = 0.5,
) -> List[int]:
    """
    Sort indices from left to right using x, and if two objects are almost
    vertically aligned, break ties by y from top to bottom.

    Here, "almost vertically aligned" means their center x-coordinates differ
    by at most x_eps_factor * (a typical object width),
    where we use the max width of the two objects being compared.

    Args:
        centroids: list of (cx, cy) for each object.
        widths: list of object widths (same order/length as centroids).
        x_eps_factor: fraction of object width used as vertical-alignment threshold.
                      e.g. 0.5 = within half the wider object's width.

    Returns:
        A list of indices in the desired order.
    """
    if not centroids:
        return []

    assert len(centroids) == len(widths), "centroids and widths must have same length"

    # First, sort all indices by x (from left to right)
    idxs = list(range(len(centroids)))
    idxs.sort(key=lambda i: centroids[i][0])
    ...
    for idx in idxs[1:]:
        ...
        cx, _ = centroids[idx]
        prev_cx, _ = centroids[prev]

        # local threshold based on the two objects' widths
        local_eps = x_eps_factor * max(widths[idx], widths[prev])

        if abs(cx - prev_cx) <= local_eps:
            # almost vertically aligned → same group
    ...
    # Within each group, sort by y from top to bottom (smaller y first)
    ordered: List[int] = []
    for g in groups:
        g_sorted = sorted(g, key=lambda i: centroids[i][1])
        ordered.extend(g_sorted)
...
def sort_indices_strict_left_to_right(
    ...
    if not centroids:
        return []
    return sorted(range(len(centroids)), key=lambda i: centroids[i][0])
...
class OneOrMorePeople(Question):
    def __init__(self) -> None:
        super().__init__(
            question="Are there one or more people in the image? Respond with 'Yes' or 'No'.",
            variables=[],
            predicates=[
...
class PersonAtClosestBench(Question):
    def __init__(self) -> None:
        super().__init__(
            question=(
                "Are there people at the bench closest to the clock? Respond with 'Yes' or 'No'. "
            ),
            variables=[],
            predicates=[
    ...
    def apply(self, image, detections):
        ...
        # If no benches, no persons, or no bus, we don't generate this question
        if len(benches) == 0 or len(persons) == 0 or len(clocks) == 0:
            return []
        ...
        bench_distances = [
            np.linalg.norm(np.array(bc) - np.array(bus_centroid))
            for bc in bench_centroids
        ]
        closest_bench_idx = int(np.argmin(bench_distances))

        # Assign each person to the nearest bench
        bench_occupancy = assign_objects_to_location(
            person_centroids, bench_centroids
        )

        answer = "Yes" if bench_occupancy[closest_bench_idx] > 0 else "No"
...
class ClosestBenchWithPerson(Question):
    def __init__(self) -> None:
        super().__init__(
            question=(
                ...
                "Which bench is closest to the clock that has at least one person at it? Answer the bench ID"
    ...
    def apply(self, image, detections):
        ...
        bench_order_by_distance = sorted(
            range(len(benches)), key=lambda i: bench_distances[i]
        )
        ...
        for idx in bench_order_by_distance:
            if bench_occupancy[idx] > 0:
                # Return the left-to-right bench number (with tie-break rule)
...
class ClosestToFurthestBenches(Question):
    def __init__(self) -> None:
        ...
        Benches are first numbered from left to right (with a vertical
        tiebreak when they are almost aligned horizontally), and the
        answer lists these bench numbers from closest to farthest
        from the bus.
        """
        super().__init__(
            question=(
    ...
    def apply(self, image, detections):
        ...
        # Compute distance from each bench to the bus
        bench_distances = [
            np.linalg.norm(np.array(bench_centroids[i]) - np.array(bus_centroid))
            for i in range(len(benches))
        ]

        # Sort bench indices by distance to the bus (closest → farthest),
        # and then convert to the left-to-right bench numbers.
        idx_by_distance = [
            bench_num[idx]
            for idx in sorted(range(len(benches)), key=lambda i: bench_distances[i])
        ]
...
class ClosestToFurthestStopSigns(Question):
    ...
    def apply(self, image, detections):
        """
        Number stop signs from left to right (breaking near-horizontal ties
        by vertical position), then rank these numbered signs from closest to
        furthest from the bus (clock) by Euclidean distance between centroids.
        """
        ...
        # Number stop signs from left to right, with vertical tie-break
        ...
        # Compute distances to the bus
        stop_sign_distances = [
            np.linalg.norm(np.array(stop_sign_centroids[i]) - bus_c)
            for i in range(len(stop_signs))
        ]

        # Sort all stop signs by distance (closest → furthest), then convert to IDs
        idx_by_distance = sorted(
            range(len(stop_signs)), key=lambda i: stop_sign_distances[i]
        )
...
def recommend_detour_direction(
    ...
    """
    推荐绕行方向：
    - 若 blocking_obstacles 为空，返回 "keep straight"
    - 否则选沿 bus→target 线段方向上“最先挡住”的 main_obstacle：
        t 最小的那个 bbox（segment_box_entry_t）。
    - 若 |dx| >= |dy| （整体偏水平行进）：
        在 up / down 中选择，非伸展方向是宽度固定 150px 的竖直条带。
    - 若 |dx| <  |dy| （整体偏竖直行进）：
        在 left / right 中选择，非伸展方向是高度固定 150px 的水平条带。

    条带厚度：150 像素（half = 75），以 main_obstacle 的中心为中线。
    """

    if not blocking_obstacles:
        return "keep straight"

    # 线段端点：从 bus 中心指向 target 中心
    bus_c = np.array(centroid(bus_bbox))
    tgt_c = np.array(centroid(target_bbox))
    dx = tgt_c[0] - bus_c[0]
    dy = tgt_c[1] - bus_c[1]
    ...
    for ob in blocking_obstacles:
        t = segment_box_entry_t(bus_c, tgt_c, ob)
        if t is None:
            continue
        if t < best_t:
            best_t = t
    ...
    # 兜底：如果上面都 None，就退回最近距离策略
    if best_obstacle is None:
        def center(b):
            return np.array(centroid(b))
        dists = [np.linalg.norm(center(ob) - bus_c) for ob in blocking_obstacles]
        main_obstacle = blocking_obstacles[int(np.argmin(dists))]
    else:
        main_obstacle = best_obstacle

    x1_m, y1_m, x2_m, y2_m = map(float, main_obstacle)
    main_c = np.array([(x1_m + x2_m) / 2.0, (y1_m + y2_m) / 2.0])

    # 非伸展方向固定厚度 150 像素
    CORRIDOR_HALF = 75.0  # 半厚度

    def horizontal_clearances():
        """
        在需要 left / right 二选一时使用：返回 (clearance_left, clearance_right)。

        通道形状（高度固定 150px）：
        - 左侧通道： [0, x1_m] × [cy-75, cy+75]
        - 右侧通道： [x2_m, image_width] × [cy-75, cy+75]

        只考虑与该 150px 高度带在 y 方向上有重叠的障碍。
        """
        cy = main_c[1]
        corridor_y_min = max(0.0, cy - CORRIDOR_HALF)
        corridor_y_max = min(float(image_height), cy + CORRIDOR_HALF)

        clearance_left = x1_m - 0.0
        clearance_right = image_width - x2_m

        for ob in all_obstacles:
            if ob is main_obstacle:
                continue
            ox1, oy1, ox2, oy2 = map(float, ob)

            # 必须与通道高度带有重叠
            if oy2 <= corridor_y_min or oy1 >= corridor_y_max:
                continue

            # 障碍在左侧：用其右边界 ox2
            if ox2 <= x1_m:
                dist = x1_m - ox2
                if dist < clearance_left:
                    clearance_left = dist

            # 障碍在右侧：用其左边界 ox1
            if ox1 >= x2_m:
                dist = ox1 - x2_m
                if dist < clearance_right:
                    clearance_right = dist

        return clearance_left, clearance_right

    def vertical_clearances():
        """
        在需要 up / down 二选一时使用：返回 (clearance_up, clearance_down)。

        通道形状（宽度固定 150px）：
        - 上侧通道： [cx-75, cx+75] × [0, y1_m]
        - 下侧通道： [cx-75, cx+75] × [y2_m, image_height]

        只考虑与该 150px 宽度带在 x 方向上有重叠的障碍。
        """
        cx = main_c[0]
        corridor_x_min = max(0.0, cx - CORRIDOR_HALF)
        corridor_x_max = min(float(image_width), cx + CORRIDOR_HALF)

        clearance_up = y1_m - 0.0
        clearance_down = image_height - y2_m
        ...
        for ob in all_obstacles:
            ...
            # 必须与通道宽度带有重叠
            if ox2 <= corridor_x_min or ox1 >= corridor_x_max:
                continue

            # 障碍在上方：用其下边界 oy2
            if oy2 <= y1_m:
                dist = y1_m - oy2
                if dist < clearance_up:
                    clearance_up = dist

            # 障碍在下方：用其上边界 oy1
            if oy1 >= y2_m:
                dist = oy1 - y2_m
                if dist < clearance_down:
                    clearance_down = dist

        return clearance_up, clearance_down

    # —— ② 根据主行进方向决定是选 up/down 还是 left/right ——

    # 水平占主导：在 up / down 里选
    if abs(dx) >= abs(dy):
        clearance_up, clearance_down = vertical_clearances()

        if clearance_up < clearance_down:
            # 上边更早撞到障碍/墙 → 上侧通道更窄 → 走下边
            return "go down"
        elif clearance_down < clearance_up:
            # 下边更早撞到 → 下侧通道更窄 → 走上边
            return "go up"
        else:
            # 宽度一样，按目标相对 main_obstacle 的位置决策
            if tgt_c[1] >= main_c[1]:
                return "go down"
            else:
                return "go up"

    # 垂直占主导：在 left / right 里选
    else:
        clearance_left, clearance_right = horizontal_clearances()

        if clearance_left < clearance_right:
            # 左边更早撞到 → 左侧更窄 → 走右边
            return "go right"
        elif clearance_right < clearance_left:
            # 右边更窄 → 走左边
            return "go left"
        else:
            if tgt_c[0] >= main_c[0]:
...
class PathBlockedBetweenBusAndBench(Question):
    def __init__(self) -> None:
        ...
        Answer format:
        - One of: "keep straight", "go left", "go right", "go up", "go down".
        """
        super().__init__(
            question=(
                "Each bench in the image has a visible number label beside it (e.g., 1, 2, 3, ...). Use these printed numbers as the bench IDs. "
                "For bench number {bench_number}, choose the best direction for the clock to move to reach that bench while avoiding obstacles between them. "
                "Answer with exactly one of: 'keep straight', 'go left', 'go right', 'go up', or 'go down'. "
                "Answer 'keep straight' if the clock can move directly toward bench {bench_number} without colliding with any obstacle. "
                "Answer 'go left' or 'go right' if the bench is mainly above or below the clock and it is better for the clock to pass the nearest obstacle between them on its left or right side. "
                "Answer 'go up' or 'go down' if the bench is mainly to the left or right of the clock and it is better for the clock to pass the nearest obstacle between them above or below it."
            ),
            variables=["bench_number"],
            predicates=[
                ObjectDetectionPredicates.at_least_one_single_detection,
    ...
    def apply(self, image, detections):
        ...
        for pc in person_centroids:
            if len(benches) == 0:
                person_assignment.append(-1)
            else:
                dists = [
                    np.linalg.norm(np.array(pc) - np.array(bc))
                    for bc in bench_centroids
                ]
                nearest_idx = int(np.argmin(dists))
                person_assignment.append(nearest_idx)
...
class PathBlockedBetweenBusAndStopSign(Question):
    def __init__(self) -> None:
        """
        ...
        super().__init__(
            question = (
                "Each stop sign in the image has a visible number label beside it (e.g., 1, 2, 3, ...). Use these printed numbers as the stop sign IDs. "
                "For stop sign number {stop_sign_number}, choose the best direction for the clock to move to reach that stop sign while avoiding obstacles between them. "
                "Answer with exactly one of: 'keep straight', 'go left', 'go right', 'go up', or 'go down'. "
                "Answer 'keep straight' if the clock can move directly toward stop sign {stop_sign_number} without colliding with any obstacle. "
                "Answer 'go left' or 'go right' if the stop sign is mainly above or below the clock and it is better for the clock to pass the nearest obstacle between them on its left or right side. "
                "Answer 'go up' or 'go down' if the stop sign is mainly to the left or right of the clock and it is better for the clock to pass the nearest obstacle between them above or below it."
            ),
            variables=["stop_sign_number"],
            predicates=[
                ObjectDetectionPredicates.at_least_one_single_detection,
    ...
    def apply(self, image, detections):
        ...
        animal_assignment: list[int] = []
        for ac in animal_centroids:
            if len(stop_signs) == 0:
                animal_assignment.append(-1)
            else:
                dists = [
                    np.linalg.norm(np.array(ac) - np.array(sc))
                    for sc in stop_sign_centroids
                ...
                nearest_idx = int(np.argmin(dists))
                animal_assignment.append(nearest_idx)
